...
<script>
    ...
    $('.update_app_status').click(function(){
        var id = $(this).attr('data-id');
        var status = $(this).attr('data-status');
        update_app_status(id, status);
    });

    function update_app_status(id, status){
        start_load();
        $.ajax({
            url:'ajax.php?action=update_appointment_status',
            method:'POST',
            ...
            success:function(resp){
                if(resp.status == 1){
                    ...
                    alert_toast("Failed to update appointment status", 'error');
                }
                end_load();
            }
        });
